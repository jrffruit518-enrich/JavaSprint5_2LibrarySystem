...
public class DataInitializerForH2 implements CommandLineRunner {
    ...
    private void initUsers() {
        // --- 1. Root Admin Initialization ---
        // Ensure the absolute root administrator exists
        if (!userRepository.existsByUsername("admin")) {
            User admin = new User();
            admin.setUsername("admin");
            // Set password to 111111 as requested
            ...
            System.out.println(">> H2: Root admin initialized (admin/111111).");
        }

        // --- 2. Test Users Initialization ---
        // Only add extra test users if the database is otherwise empty of users besides admin
        if (userRepository.count() <= 1) {
            ...
            System.out.println(">> H2: Test users 'alice' and 'bob' created.");
        }
    }
